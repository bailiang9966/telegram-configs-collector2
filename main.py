...
try:
    ...
    print(f"Successfully downloaded and moved {filename}")
except Exception as e:
    print(f"Failed to download geoip database: {e}")
    # Continue execution without the geoip file
...
try:
    ...
    print(f"Error writing no-match file: {e}")
...
try:
    ...
    print(f"Error reading last update file: {e}. Using default time.")
    last_update_datetime = datetime.now(tz=timezone(timedelta(hours=3, minutes=30))) - timedelta(days=1)
...
try:
    ...
    print(f"Error writing last update file: {e}")
    current_datetime_update = datetime.now(tz = timezone(timedelta(hours = 3, minutes = 30)))
    jalali_current_datetime_update = jdatetime.datetime.now(tz = timezone(timedelta(hours = 3, minutes = 30)))
...
def json_load(path):
    try:
        ...
        print(f"JSON decode error in {path}: {e}. Returning empty list.")
        return []
    except Exception as e:
        print(f"Error loading {path}: {e}. Returning empty list.")
        return []
...
def tg_channel_messages(channel_user):
    try:
        ...
        response = requests.get(f"https://t.me/s/{channel_user}", timeout=30)
        ...
        print(f"Network error for channel {channel_user}: {e}")
        return []
    except Exception as exc:
        print(f"Error processing channel {channel_user}: {exc}")
        return []


def process_channel(channel_user, last_update_datetime):
    """Process single channel and return results"""
    try:
        ...
        if len(div_messages) == 0:
            return {
                ...
                'removed': True,
                'messages': [],
                'check_messages': (channel_user, div_messages)
            }
        ...
        for div_message in div_messages:
            ...
            if datetime_object > last_update_datetime:
                ...
                channel_messages.append((channel_user, div_message))
        ...
        return {
            ...
            'removed': False,
            'messages': [],
            'check_messages': (channel_user, [])
        }


def process_new_channel(channel_user, invalid_array_channels):
    """Process new channel and return results"""
    if channel_user in invalid_array_channels:
        return {
            'channel': channel_user,
            'messages': [],
            'skipped': True
...
# Use ThreadPoolExecutor for parallel processing of channels
print("Processing channels in parallel...")
with ThreadPoolExecutor(max_workers=10) as executor:
    # Submit all channel processing tasks
    future_to_channel = {
        executor.submit(process_channel, channel_user, last_update_datetime): channel_user 
    ...
    for future in as_completed(future_to_channel):
        try:
            ...
            if result['removed']:
                removed_channel_array.append(result['channel'])
            
            # Check configuration Telegram channels
            channel_check_messages_array.append(result['check_messages'])
            
            # Add messages to array
            channel_messages_array.extend(result['messages'])
            
        except Exception as exc:
            channel_user = future_to_channel[future]
            print(f"Error processing channel {channel_user}: {exc}")
            continue
...
url = 'https://raw.githubusercontent.com/mirzaeisoroush/telegram-proxies-collector/main/telegram channels.json'
...
try:
    ...
    print("Successfully downloaded and processed telegram proxies channels")
except Exception as e:
    print(f"Failed to download telegram proxies channels: {e}")
    tg_username_list.update(array_usernames)
...
# Use ThreadPoolExecutor for parallel processing of new channels
print("Processing new channels in parallel...")
with ThreadPoolExecutor(max_workers=10) as executor:
    # Submit all new channel processing tasks
    future_to_channel = {
        executor.submit(process_new_channel, channel_user, invalid_array_channels): channel_user 
    ...
    for future in as_completed(future_to_channel):
        try:
            ...
            if not result['skipped'] and result['messages']:
                new_channel_messages.append((result['channel'], result['messages']))
                
        except Exception as exc:
            channel_user = future_to_channel[future]
            print(f"Error processing new channel {channel_user}: {exc}")
            continue
...
for channel, messages in new_channel_messages:
    ...
    # Use ThreadPoolExecutor for parallel processing of additional new channels
    if tg_username_list_new:
        print("Processing additional new channels in parallel...")
        with ThreadPoolExecutor(max_workers=10) as executor:
            # Submit all additional new channel processing tasks
            future_to_channel = {
                executor.submit(process_new_channel, channel_user, invalid_array_channels): channel_user 
            ...
            for future in as_completed(future_to_channel):
                try:
                    result = future.result()
                    # Note: These results are not added to new_channel_messages as per original logic
                    
                except Exception as exc:
                    channel_user = future_to_channel[future]
                    print(f"Error processing additional new channel {channel_user}: {exc}")
                    continue
...
try:
    ...
    print(f"Error writing telegram channels file: {e}")
...
try:
    ...
    print(f"Error writing invalid telegram channels file: {e}")
...
def html_content(html_address):
    try:
        ...
        print(f"Network error for URL {html_address}: {e}")
        return ""
    except Exception as e:
        print(f"Error processing URL {html_address}: {e}")
        return ""
...
for url_link in subscription_links:
    try:
        ...
        if 'mirzaeisoroush' not in url_link:
            raw_array_links_content.append((url_link, links_content))
        elif 'mirzaeisoroush' in url_link and 'channels' in url_link:
